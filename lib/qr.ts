 ...
 *
 * SCOPE: byte mode, all forty versions, all four error-correction levels — up
 * to 2953 bytes. Byte mode alone because it carries anything, including UTF-8,
 * and one code path is one code path to get right. A payload too long for
 * version 40 throws rather than silently producing a code that will not scan.
 *
...
/** Total codewords (data + error correction) per version, index 1-40. */
 ...
 *
 * From ISO/IEC 18004 Annex E. A version-1 test proves nothing about version 27,
 * so tests/qr.test.ts round-trips a payload at every one of the forty.
 */
