...
It states **checkable facts** about the order — number, date placed, total,
whether payment was received and when, current delivery status, and who sold it —
read live at the moment somebody looks. A reader holding a printed invoice can
compare those against the paper.

It **certifies nothing**. There is no badge, no score, no "verified merchant"
mark, because a shop awarding itself one is worth exactly nothing and reads as
though the shop knows it. The only thing of value is that the page and the
document agree, which is a fact the reader establishes rather than a claim we
make.

**Privacy:** `getReceiptByNumber` returns a redacted order to anyone who has not
proved it is theirs — the email is masked and the delivery address is stripped
entirely. So an order number is enough to confirm a document and not enough to
...
nothing at all, and assertions about size and finder patterns pass happily on
one. The first draft had exactly two such bugs: the format bits were written
LSB-first instead of MSB-first, and the version-information BCH loop ran twelve
times instead of six (which only shows from version 7, where the version block
first appears).

So `tests/qr.test.ts` compares whole matrices against fixtures generated by an
independent implementation, at a matching mask. Mask *selection* is allowed to
differ — the two implementations score the symbol slightly differently, both
